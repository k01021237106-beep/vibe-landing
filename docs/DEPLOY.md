...
### ✅ 메일 발송(SMTP) — 2026-09-07 완료

- [x] **도메인 구입 + SPF·DKIM 설정** — `firstdeploy.kr` (아래 '도메인과 DNS')
- [x] **자체 SMTP 연결** — Resend. 설정값은 아래 표.
- [x] **수신 시험** — 지메일·네이버 받은편지함 도착 (다음은 미확인, 아래 참고)
- [x] **로그인 메일 한국어화** — Magic Link, Confirm signup 두 템플릿 모두
...
| Password | Resend API 키 (`supabase-auth`, Sending access 권한) |

⚠️ **Sender email의 도메인은 `send.firstdeploy.kr`이어야 한다.**
...
| 수신처 | 결과 |
|---|---|
| 지메일 (기존 회원) | 받은편지함. SPF·DKIM·DMARC **전부 PASS** |
| 네이버 (새 가입) | 받은편지함 |
| 지메일 `+test1` (새 가입) | 받은편지함. 계정 생성 → 59초 뒤 로그인 |
| 다음(Daum) | **미확인** — 계정이 없어 시험하지 못했다. 안 되는 것이 아니라 안 해 본 것이다 |

#### 메일 템플릿은 **두 개**를 고쳐야 한다
...
Magic Link만 한국어로 바꾸고 끝난 줄 알았는데, 새 손님이 받는 첫 메일은
여전히 영문 `Confirm your email address`였다.
**손님이 우리에게서 받는 첫 메일이 가장 중요한데 그쪽이 남아 있었다.**

> 새 가입 흐름을 시험할 때는 지메일 `+` 별칭을 쓰면 된다 —
> `k01021237106+test1@gmail.com`. 지메일은 원래 받은편지함에 넣어 주고,
> Supabase는 다른 주소로 보아 새 계정을 만든다. 메일 계정을 더 만들 필요가 없다.
>
> ⚠️ 브라우저 자동완성이 `+test1`을 지우고 저장된 주소로 채우는 일이 있다.
> **「메일을 보냈습니다」 화면에 뜨는 주소를 먼저 확인**한다.
> 거기에 `+test1`이 없으면 메일함을 볼 필요도 없다.
