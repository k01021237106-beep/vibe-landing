...
## 🔴 내일 여기서 시작 (2026-09-08 저녁 기준)

**막힌 자리: OBS 녹화 파일에 목소리가 안 들어간다.**

증상 — OBS 믹서 막대는 말할 때 움직이는데, 녹화된 MP4는 **완전 무음**이다.
막대가 움직인다는 건 **OBS가 소리를 받고는 있다**는 뜻이다. 받아서 녹화로
안 보내고 있다.

이미 확인한 것 (전부 정상이었다):
K66 USB 마이크 Windows 인식 · Windows 마이크 테스트 반응 · OBS 믹서 막대 움직임 ·
장치가 K66으로 지정됨 · 녹화 오디오 트랙 1 체크 · 고급 오디오 속성에서도 트랙 1 ·
음소거 아님

**아직 안 해 본 1순위** — 오디오 믹서에서 **마이크/Aux 우클릭 → 고급 오디오 속성 →
「오디오 모니터링」 칸**을 본다.
...
**「모니터링 끄기」로 바꾸고 10초 녹화해서 재생**한다.

> **왜 여기가 1순위인가.** 내장 마이크 때는 소리가 들어가긴 했다(작았을 뿐).
> K66을 붙이고 **모니터링 출력 장치를 스피커로 지정한 뒤부터** 완전 무음이 됐다.
> 그 근처를 만지다 모니터링 방식까지 바뀌었을 가능성이 높다.

안 되면 다음 순서: ② 볼륨 슬라이더가 바닥인지 → ③ 다른 플레이어로 재생 →
④ 믹서에 오디오 소스가 하나 더 있는지 → ⑤ **장면에 「오디오 입력 캡처」를 새로
추가**해 기본값으로 다시 만든다 (꼬인 설정을 통째로 건너뛴다).

**해결되면 `docs/course/recording-checklist.md`에 기록한다.**
