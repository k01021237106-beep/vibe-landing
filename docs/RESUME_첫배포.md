...
**메일과 로그인은 끝났다.** 우리 도메인(`hello@send.firstdeploy.kr`)에서 보낸
한국어 메일로, 처음 오는 손님과 다시 오는 손님 둘 다 로그인이 확인됐다.

**사업자등록을 접수했고(2026-09-08), 개업일은 10월 1일이다.**
그래서 **유료 판매 시작 가능일이 10월 1일**이고, 그때까지 3주가 준비 기간이다.

지금 하는 일은 **무료 1강 영상 제작**이다.
등록증이 나오기 전까지 유료화도 `main` 공개도 서두르지 않는다.
...
| 네이버 수신 (새 가입) | ✅ 받은편지함 | — |
| SPF·DKIM·DMARC | ✅ 전부 PASS | 지메일 ⋮ → 원본 보기 |
| 로그인 메일 한국어화 | ✅ 템플릿 2개 | Magic Link + Confirm signup |
| 새 손님 전 구간 | ✅ 2026-09-07 04:27 | 아래 '메일과 로그인' 참고 |
| 다음(Daum) 수신 | ⬜ **미확인** | 계정이 없어 시험 못 함. 안 되는 게 아니다 |
...
### ⑤⑥ 메일 한국어화 + 수신 시험 — ✅ 끝났다 (2026-09-07)

#### 메일 템플릿은 **두 개**다

이걸 놓쳐서 한 번 헤맸다. Supabase는 상황에 따라 다른 템플릿을 쓴다:

| 손님 | 나가는 템플릿 |
|---|---|
| 이미 있는 회원이 로그인 | **Magic Link** |
| 처음 오는 손님 | **Confirm signup** |

Magic Link만 고치고 끝난 줄 알았는데, 새 손님이 받는 첫 메일은 여전히
영문 `Confirm your email address`였다. 둘 다 고쳐야 한다.

#### 새 손님 전 구간이 실제로 돌았다

```
k01021237106+test1@gmail.com
  계정 생성   2026-09-07 04:27:47
  로그인      2026-09-07 04:28:46   ← 59초 뒤 (메일 왕복 시간)
  권한        student   (admin 아님)
  수강권      0건       (결제 전이니 맞다)
```

> 새 가입 흐름은 지메일 **`+` 별칭**으로 시험한다 — `...+test1@gmail.com`.
> 지메일은 원래 받은편지함에 넣어 주고, Supabase는 다른 주소로 보아
> 새 계정을 만든다. 메일 계정을 더 만들 필요가 없다.
>
> ⚠️ 브라우저 자동완성이 `+test1`을 지우고 저장된 주소로 채우는 일이 있다.
> **「메일을 보냈습니다」 화면의 주소를 먼저 확인**한다.
> 실제로 이것 때문에 시험이 한 번 헛돌았다 (계정이 안 생겼는데 성공처럼 보였다).

#### 남은 시험 계정

| 주소 | 상태 | 쓸모 |
|---|---|---|
| `kk024199@naver.com` | 오타. 미인증, 로그인 이력 없음 | 없음 — 지워도 된다 |
| `kko24199@naver.com` | 정상 (student) | 손님 시점 확인용으로 쓸 만하다 |
| `k01021237106+test1@gmail.com` | 정상 (student) | 손님 시점 확인용으로 쓸 만하다 |

student 계정이 있으면 `/admin`이 안 보이는 것,
결제 전 `/my`가 어떻게 보이는지를 실제로 확인할 수 있다. 그래서 남겨 두었다.
...
⑥까지 끝났으므로 이제 이 결정만 남았다. 판매 전 남은 항목은
...
## 4-B. 사업자등록 (2026-09-08 접수)

| 항목 | 값 |
|---|---|
| 상호 | **멜라빈 스튜디오** |
| 개업일 | **2026-10-01** ← 유료 판매 시작 가능일 |
| 주업종 | 940306 1인미디어콘텐츠창작자 |
| 상태 | 홈택스 접수 완료, 등록증 대기 |

### ⚠️ 등록증이 나오면 확인할 것

**940306만으로 통신판매업 신고가 되는지.** 이 코드는 유튜버·크리에이터용이고,
우리는 사이트에서 직접 결제받는 전자상거래다.
통신판매업 신고나 토스 계약에서 **전자상거래 소매업** 계열을 요구할 수 있다.
반려되면 사업자등록 정정부터 다시 해야 하므로, 등록증을 받는 즉시 물어본다.
...
4. 샘플 후기 처리 (실제 후기가 없으니 섹션 내리기를 권함)
5. 강사 소개 자료 (사진 + 실제 이력)
6. 강의 가격 최종 확정 (지금 198,000 → 99,000)
7. 법무 검토 의뢰 (번호는 나중에 채워도 검토는 지금 가능)

### 등록번호가 나온 뒤 (순서대로)

1. 업종 확인 (위 ⚠️)
2. **토스 계약 → 구매안전서비스 이용확인증 받기**
3. **통신판매업 신고** (정부24, 이용확인증 필요, 등록면허세 약 4만원)
4. 토스 라이브 키 발급 → Vercel 교체 → 실결제 확인
5. `business` 값 채우기, 법적 고지에 번호·시행일 반영
6. 카카오 비즈앱 전환 → 카카오 로그인 켜기
7. `main` 병합 = 공개

> 2번과 3번은 서로 기다린다. 토스에서 이용확인증을 받아 정부24에 내는 순서가 보통 빠르다.
